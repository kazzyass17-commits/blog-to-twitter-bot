...
- **承認時間**: 数分〜数日（通常は数分で承認）
- **コスト**: **無料**（Basicプランで十分です）
  - Basicプランは無料で利用できます
  - このプロジェクト（1日3回の投稿）ではBasicプランで十分です
  - 有料プラン（Pro、Enterprise）は不要です
...
#### 無料で使用可能
- **GitHub Actions**: 無料プラン（1ヶ月あたり2,000分まで）
- **Twitter API**: Basicプラン（無料）
- **データベース**: SQLite（ファイルベース、無料）

#### 有料になる場合
- GitHub Actionsの使用量が2,000分/月を超える場合
  - このプロジェクトでは1日3回×約5分/回 = 約15分/日
  - 1ヶ月で約450分なので、無料プランで十分です
- Twitter APIの有料プランが必要な場合
  - **このプロジェクトでは不要です**（Basicプランで十分）
  - 有料プラン（Pro、Enterprise）は高トラフィックや商用利用向けです
...
#### ネットワーク接続
- **必須**: インターネット接続が必要です
- **注意**: VPNを使用している場合、接続が不安定になる可能性があります

#### ブログの利用規約
- **注意**: ブログの利用規約を確認してください
- **注意**: 自動取得が許可されているか確認してください

#### Twitter APIの利用規約
- **注意**: Twitter APIの利用規約を遵守してください
- **注意**: スパム行為や過度な投稿は避けてください
- **注意**: 1日3回の投稿は問題ありませんが、それ以上は注意が必要です

#### データのバックアップ
- **推奨**: `posts.db`ファイルを定期的にバックアップしてください
- **注意**: データベースが破損した場合、再初期化が必要です

#### ブログの構造変更
- **注意**: ブログのHTML構造が変更されると、URL抽出が失敗する可能性があります
- **対応**: その場合は`index_extractor.py`を修正する必要があります

#### タイムゾーンの設定
- **注意**: GitHub ActionsはUTC時間で実行されます
- **注意**: JST（日本時間）で8時、14時、20時に実行するように設定済み
- **変更**: タイムゾーンを変更する場合は、`.github/workflows/scheduled_posts.yml`を修正
