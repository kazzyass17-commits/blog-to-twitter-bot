...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bot_pursahs.log', encoding='utf-8'),
...
def main():
    """メイン関数 - pursahsgospel専用"""
    logger.info("=" * 60)
    logger.info("ブログ→Twitter自動投稿ボット開始 (pursahsgospel)")
    ...
    try:
        ...
        if not credentials_pursahs.get('api_key') or not credentials_pursahs.get('access_token'):
            ...
            logger.error(".envファイルにTWITTER_PURSAHS_API_KEY、TWITTER_PURSAHS_ACCESS_TOKENなどを設定してください")
            return False
        ...
        # 結果サマリー
        logger.info("\n" + "=" * 60)
        logger.info("処理完了 (pursahsgospel)")
