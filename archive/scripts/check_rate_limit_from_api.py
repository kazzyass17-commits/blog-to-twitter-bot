...
def check_both_accounts():
    """両方のアカウントのレート制限情報を確認し、rate_limit_state.jsonを更新"""
    print("=" * 60)
            ...
            if result.get('rate_limited'):
                ...
                if result.get('reset_time'):
                    ...
                    record_rate_limit_reason(
                        ...
                        error_message='APIから取得したレート制限情報',
                        reset_time=reset_time,
                        wait_until=wait_until,
                        rate_limit_limit=result.get('limit'),
                        rate_limit_remaining=result.get('remaining')
                    )
                    print(f"  ✓ rate_limit_state.jsonを更新しました")
            else:
