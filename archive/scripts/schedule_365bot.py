...
def run_scheduled_task():
    """スケジュールされたタスクを実行"""
    logger.info("=" * 60)
    logger.info("スケジュール実行を開始します (365botGary)")
...
def schedule_daily_posts():
    ...
    for hour, minute in times:
        ...
        logger.info(f"スケジュール登録: 毎日 {hour:02d}:{minute:02d} (365botGary)")
    
    logger.info(f"合計{len(times)}件のスケジュールを登録しました (365botGary)")


def start_scheduler():
    """スケジューラーを開始"""
    logger.info("=" * 60)
    logger.info("スケジューラーを開始します (365botGary)")
    logger.info("投稿時間: 8時、14時、20時（各時間帯でランダムな分）")
    logger.info("=" * 60)
    
    # 初回実行（起動時）
    logger.info("初回実行を行います (365botGary)")
    run_scheduled_task()
    ...
    logger.info("次の投稿スケジュール (365botGary):")
    ...
    try:
        while True:
            ...
            if now.hour == 0 and now.minute == 0:
                logger.info("新しい日のスケジュールを再生成します (365botGary)")
                schedule.clear()
                schedule_daily_posts()
    except KeyboardInterrupt:
        logger.info("\nスケジューラーを停止します (365botGary)")
