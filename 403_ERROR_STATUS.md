...
### 1. テキストの長さや内容による制限（可能性が高い）

- 診断スクリプト（短いテキスト）: ✅ 成功
- 本番形式のテスト投稿（長いテキスト）: ❌ 403エラー
- あとからまとめて投稿されていたケースがある → レート制限ではない

**考えられる原因:**
- 長いテキスト（280文字近く）の投稿に特別な制限がある可能性
- テキストの内容によるフィルタリング
- URLを含む長いテキストの投稿に制限がある可能性
...
**推奨される対処法：**
1. **テキストの長さを調整**
   - 短いテキストでは成功しているため、テキストの長さを調整してテスト
   - タイトルを削除して本文のみで投稿するように変更済み（より多くの本文を引用可能）
